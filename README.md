...
An [OpenCode](https://github.com/opencode-ai/opencode) plugin that automatically runs linters on modified files after each tool execution turn. This helps catch code quality issues immediately while you're working, providing instant feedback without interrupting your flow.
...
This plugin requires the following tools to be installed on your system, depending on which languages you're linting:
...
Add the plugin to your OpenCode configuration. The exact method depends on your OpenCode setup, but typically you'll need to:
...
Create a configuration file at `.opencode/linter.config.json` in your project root:
...
You can extend this plugin or create your own based on the same patterns. The plugin exports a `PostTurnLinter` function that follows the OpenCode plugin interface.
...
7. **Reporting**: Results are sent back to the OpenCode session as a prompt with all linter output
...
2. Verify your configuration file exists at `.opencode/linter.config.json`
